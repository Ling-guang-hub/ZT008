...
public class SaveDataManager
{
    ...
    public static void SetSimpleDataList<T>(string key, List<T> list)
    {
        for (int i = 0; i < list.Count; i++)
        {
            PlayerPrefs.SetString(key + "SimpleList" + i, list[i].ToString());
        }
        
        PlayerPrefs.SetInt(key + "SimpleListCount", list.Count);
    }


    public static List<string> GetSimpleDataList(string key)
    {
        List<string> list = new List<string>();
        int lengthNum = PlayerPrefs.GetInt(key + "SimpleListCount");

        for (int i = 0; i < lengthNum; i++)
        {
            list.Add(PlayerPrefs.GetString(key + "SimpleList" + i));
        }
