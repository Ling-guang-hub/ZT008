...
public class MusicMgr : MonoSingleton<MusicMgr>
{
    ...
    {
        get { return BgMusicSwitch ? getVolume(BGM_Name) : 0f; }
        set
        {
            _BgVolume = value;
            //背景音乐开的状态下，声音随控制调节
    ...
    private async void PlayEffectBase(object effectName, bool defAudio = true, float volume = 1f, float durTime = 0f)
    {
        ...
        if (defAudio)
        {
            ...
            if (durTime > 0)
            {
                await UniTask.Delay((int)(durTime * 1000), DelayType.Realtime);
                m_effectMusic.Stop();
            }
        }
        else
    ...
    public void PlayEffect(MusicType.UIMusic effectName, float time, bool defAudio = true, float volume = 1f)
    {
        PlayEffectBase(effectName, defAudio, volume, time);
    }
