...
namespace ScratchCardAsset
{
    ...
    public class EraseProgress : MonoBehaviour
    {
        ...
        [SerializeField, FormerlySerializedAs("Card")]
        ...
        [SerializeField, FormerlySerializedAs("ProgressMaterial")]
        ...
        [SerializeField, FormerlySerializedAs("SampleSourceTexture")]
        ...
        private void LateUpdate()
        {
            ...
            if ((card.IsScratched || updateProgress) && !isCompleted)
            {
                ...
                {
                    MessageCenterLogic.GetInstance().Send(CConfig.mg_PlayScratchMusic);
        ...
        private void Init()
        {
            ...
            var textureFormat = SystemInfo.SupportsTextureFormat(TextureFormat.R8)
                ? TextureFormat.R8
                : TextureFormat.ARGB32;
            progressTexture = new Texture2D(percentRenderTexture.width, percentRenderTexture.height, textureFormat,
                false, true);
        }
            ...
            if (!isCompleted && !isCalculating)
            {
                ...
                if (progressAccuracy == ProgressAccuracy.High)
                {
                    if (!pixelsBuffer.IsCreated)
                    {
                        ...
                        pixelsBuffer = new NativeArray<byte>(length, Allocator.Persistent,
