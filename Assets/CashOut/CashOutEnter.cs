...
public class CashOutEnter : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        OpenPanelBtn.onClick.AddListener(() =>
        {
            ...
            if (!SaveDataManager.GetBool(CConfig.sv_FinishShopGuide)) return;
        ...
        MessageCenterLogic.GetInstance().Register(CConfig.mg_ShowCashOutText, (md) =>
        {
            if (IsInvoking(nameof(ShowUI)))
            {
                CancelInvoke(nameof(ShowUI));
            }
            Invoke(nameof(ShowUI), 0.2f);
        });

        MessageCenterLogic.GetInstance().Register(CConfig.mg_ShowCashTimeText, (md) => { UpdateTime(md.valueString); });
    }
    ...
    public void UpdateMoney()
    {
        ...
        int MoneyStart =Mathf.FloorToInt(  float.Parse(MoneyText.text, CultureInfo.CurrentCulture));
        MoneyTextAnim = DOTween.To(() => MoneyStart, x => MoneyText.text = x.ToString(),
            Mathf.FloorToInt(CashOutManager.GetInstance().Money), 1f);
        ...
        int MoneyEnd =Mathf.FloorToInt( CashOutManager.GetInstance().Money);
        MaxMoneyFillAnim = DOTween.To(() => MaxMoneyFill.fillAmount, x => MaxMoneyFill.fillAmount = x,
            Mathf.Min(1, MoneyEnd / MaxMoney), 1f);
    }
    ...
    public void MoneyToCashAnim(bool IconFly)
    {
        ...
        MoneyTextAnim = DOTween.To(() => MoneyStart, x => MoneyText.text = x.ToString(), 0, 1f);
        CashTextAnim = DOTween.To(() => CashOutStart, x => CashText.text = x.ToString("F2"), CashOutEnd, 1f)
            .SetDelay(.7f);
