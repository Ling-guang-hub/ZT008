...
public class FishScope : BaseUIForms
{
    public static FishScope Instance;
    ...
    [FormerlySerializedAs("newCardToastObj")]


    public GameObject newCardToastObj;
    
    [FormerlySerializedAs("collectBarKey")]

    
    public GameObject collectBarKey;

    [FormerlySerializedAs("collectBarFlame")]


    public GameObject collectBarFlame;

    [FormerlySerializedAs("collectBarLight")]


    public GameObject collectBarLight;
    ...
    [FormerlySerializedAs("flyAreaObj")]


    public GameObject flyAreaObj;
    ...
    private void DoAdaptation()
    {
        if (LocalCommonData.ScreenRate > 0.5f)
        {
            ...
            topBar.transform.localPosition = new Vector3(225f, -60f, 0);
            BoastJet.gameObject.transform.localPosition = new Vector3(-120f, -60f, 0);
            wheelBar.transform.localPosition = new Vector3(-90f, -250f, 0);
            levelBar.transform.localPosition = new Vector3(100f, -255f, 0);
            newCardToastObj.transform.localPosition = new Vector3(-200f, -255f, 0);
            

            cashShopObj.transform.localScale = new Vector3(0.8f, 0.8f, 0);
            cashShopObj.transform.localPosition = new Vector3(0f, -180f, 0);
            
            cardArea.transform.localPosition = new Vector3(0, 50, 0);

            collectBarKey.transform.localScale = new Vector3(0.9f, 0.9f, 0);
            collectBarLight.transform.localScale = new Vector3(0.9f, 0.9f, 0);
            collectBarFlame.transform.localScale = new Vector3(0.9f, 0.9f, 0);
            collectBarKey.transform.localPosition = new Vector3(25, -600f, 0);
            collectBarLight.transform.localPosition = new Vector3(200f, 600, 0);
            collectBarFlame.transform.localPosition = new Vector3(-160f, 600f, 0);

            passportBtn.gameObject.transform.localPosition = new Vector3(-150f, 160, 0);
            taskBtn.gameObject.transform.localPosition = new Vector3(150f, 160, 0);
            mainBtn.transform.localPosition = new Vector3(0, 160, 0); 
        ...
        else
        {
            ...
            BoastJet.gameObject.transform.localPosition = new Vector3(-120f, -80f, 0);
            ...
            newCardToastObj.transform.localPosition = new Vector3(-200f, -330f, 0);
            

            cardArea.transform.localPosition = new Vector3(0, 50, 0);
            collectBarKey.transform.localPosition = new Vector3(25, -650f, 0);
            collectBarLight.transform.localPosition = new Vector3(200f, 680, 0);
            collectBarFlame.transform.localPosition = new Vector3(-160f, 680f, 0);
    ...
    public override void Display(object uiFormParams)
    {
        ...
        RefreshTopData();
        HoldTimerStart();
        flyAreaObj.GetComponent<FlyPopManager>().StartFly();
    }
    ...
    private void ShowAct()
    {
        ...
        SiltPlotAll.transform.DOScale(1f, 0.2f).OnComplete(() => { InitCardData(); });
    }
    ...
    {
        CloseSomeUI();
        OpenUIForm(nameof(UtahScope));
    }

    private async void ShowCompletePanel()
    {
        int thisDaley =10;
        WinPanelData thisData = new WinPanelData()
        {
            // CoinAmount = LocalRewardData.CompleteData.CoinAmount,
            CashAmount = LocalRewardData.CompleteData.CashAmount,
            PanelType = PanelType.Card
        };

        if (LocalRewardData.CompleteData.CoinAmount > 0)
        {
            int thisCoin = LocalRewardData.CompleteData.CoinAmount;
            ...
            thisDaley = 300;
            topBar.AddCoinAndDoAnima(thisCoin, 0, false);
        }

        if (LocalRewardData.CompleteData.CashAmount > 0)
        {
            if (LocalRewardData.CompleteData.IsSpecial)
            {
                UIManager.GetInstance().ShowUIForms(nameof(ScentTheScope), thisData);
            }
            else
            {
                UIManager.GetInstance().ShowUIForms(LocalRewardData.CompleteData.CashAmount >= _bigWinLimit
                    ? nameof(BidTheScope)
    ...
    public async void AfterCollectBonus(int coinAmount, decimal cashAmount)
    {
        // ShowGuide();
        ShowItemLoopAct();
        await UniTask.Delay(300);
    ...
    private async void GoOnGame()
    {
        ...
        MessageCenterLogic.GetInstance().Send(CConfig.mg_ShowNewCardToast);
        // CheckTaskPanel();
    }
    ...
    // private async void CheckCollect01(int moreCount)
    // {
    //     if (!collectBarKey.GetComponent<CollectBarCtrl>().CheckIsFinish()) return;
    //     await UniTask.Delay(100);
    //     collectBarKey.GetComponent<CollectBarCtrl>().DoScaleAct();
    //     await UniTask.Delay(800);
    //
    //     MessageCenterLogic.GetInstance().Send(CConfig.mg_PassAnim);
    //     UIManager.GetInstance().ShowUIForms(nameof(TaleScope));
    //
    //     collectBarKey.GetComponent<CollectBarCtrl>().SetCollectCount(moreCount);
    //     collectBarKey.GetComponent<CollectBarCtrl>().ShowCollects();
    // }

    public async void CheckShowCollectBonus(bool isShowBonus, CollectType collectType, int reward, int moreCount)
    {
        if (isShowBonus)
        {
            ...
            switch (collectType)
            {
                case CollectType.KeyRing:
                    thisObj = collectBarKey;
                    break;
                case CollectType.Flame:
                    thisObj = collectBarFlame;
                    break;
                default:
                    thisObj = collectBarLight;
                    break;
            }

            await UniTask.Delay(100);
            thisObj.GetComponent<CollectBarCtrl>().DoScaleAct();
            await UniTask.Delay(800);
            ...
            if (collectType == CollectType.KeyRing)
            {
                thisData.PanelType = PanelType.Default;
                UIManager.GetInstance().ShowUIForms(nameof(TaleScope), thisData);
            }
            else
            {
                thisData.PanelType = PanelType.Default;
            ...
            thisObj.GetComponent<CollectBarCtrl>().SetCollectCount(moreCount);
            thisObj.GetComponent<CollectBarCtrl>().ShowCollects();
        }
        else
        {
            CheckShowWheelPanel();
    ...
    {
        // if (CollectManager.Instance.needFly)
        // {
        switch (CollectManager.Instance.curType)
        {
            case CollectType.KeyRing:
                collectBarKey.GetComponent<CollectBarCtrl>().AddCollectAndPlayAct();
                break;
            case CollectType.Flame:
                collectBarFlame.GetComponent<CollectBarCtrl>().AddCollectAndPlayAct();
                break;
            default:
                collectBarLight.GetComponent<CollectBarCtrl>().AddCollectAndPlayAct();
                break;
        }
        // }
    }
    ...
    public void SetMainBtn(bool isNewCard)
    {
        if (isNewCard)
        {
