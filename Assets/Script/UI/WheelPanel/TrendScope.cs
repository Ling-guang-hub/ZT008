...
public class TrendScope : BaseUIForms
{
    public static TrendScope Instance;
    ...
    void Start()
    {
        ...
        // _bigLightSkeleton.AnimationState.Complete += BoreMottle;

        playBtn.onClick.AddListener(() => { PlayWheel(); });
    }
    ...
    private void ShowAct()
    {
        ...
        SiltPlotAll.transform.DOScale(1f, 0.2f).OnComplete(() => { });
    }
    ...
    {
        FishScope.Instance.AfterWheelBonus();
    ...
    private void ShowBonusPanel()
    {
        ...
        // {
        //     OpenUIForm(nameof(TrendBidTheScope));
        // }
        // else
        // {
        //     OpenUIForm(nameof(TrendAbuseScope));
        // }
    }
    ...
    {
        bigLightObj.gameObject.SetActive(false);
        if (LocalWheelData.WheelType == CommonRewardType.Coin)
        {
            DoClose();
        }
        else
        {
            ...
            {
                CashAmount =  (int) Math.Ceiling( LocalWheelData.WheelAmount),
                PanelType = PanelType.Wheel
            };
            UIManager.GetInstance().ShowUIForms(LocalWheelData.WheelAmount >= _bigWinLimit
                ? nameof(BidTheScope)
